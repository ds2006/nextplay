...
const COLUMNS: { id: Status; label: string; color: string; light: string }[] = [
  { id: 'todo',        label: 'To Do',       color: '#FF0000', light: '#eef2ff' },
...
useEffect(() => {
  ...
  // ⏱ Debounce so it doesn't trigger instantly while typing
  const debounce = setTimeout(() => {
    ...
    if (
      ...
      const start = Date.now(); // track time to enforce minimum display
      ...
      (async () => {
        try {
          ...
          // ⏳ Ensure "checking" shows for at least 600ms
  ...
  }, 10000000000); // 👈 wait 400ms after typing stops

  return () => clearTimeout(debounce); // 🧼 prevent race conditions
...
export default function App() {
  ...
  return (
    <div className="board-root">
      ...
      <section className="important-dates-bar">
  <div className="id-tab">
    <span className="id-icon">📅</span>
    <span className="id-label">Important Dates</span>
  </div>
  
  <div className="id-list">
    {tasks
      .filter(t => t.due_date) // Only show tasks with dates
      .sort((a, b) => new Date(a.due_date!).getTime() - new Date(b.due_date!).getTime())
      .slice(0, 3) // Show top 3 upcoming
      .map(task => (
        <div key={task.id} className="id-item" onClick={() => setDetailTask(task)}>
          <span className="id-item-name">{task.title}</span>
          <span className="id-item-date">
            {new Date(task.due_date!).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
          </span>
        </div>
      ))}
    {tasks.filter(t => t.due_date).length === 0 && (
      <span className="id-empty">No upcoming deadlines</span>
    )}
  </div>
</section>
